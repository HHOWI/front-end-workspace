...
2. 동일한 디텍터리에 있는 경우 : ./파일명.확장자 or 파일명.확장자
...
2. caption : 테이블의 제목
3. thead(제목) /tfoor(결과) / tbody(내용) : 각 셀들의 역할 별 구분
...
- multiple 속성을 통해 파일을 다중으로도 선택할 수 있다.
...
- 페이지 내용상 별개의 내용으 구성할 때 사용되는 섹션

## aside

- 본문 내용에 대한 추가적인 설명, 부수적 컨텐츠 표시에 사용하는 태그
...
- 문서의 흐름에 영향을 주지 않는 컨텐츠 마크업에도 사용