...
```css
font-varioant : normal or small-caps;
```
...
- pre-warp : 여러 개의 공백을 그대로 표시, 영역을 넘어가는 내용은 자동으로 줄 바꿔 표시
...
- **no-repeat** : 배경 이미지를 한 번만 출력하고 반복하지 않음.
...
- 웹 페이지가 위 아래로 움직여도 배경이미지는 움직이지 않고 고정하는 속성
...
- 박스 모델은 콘텐츠(Content) 영역과 테두리와 콘텐츠 영역 사이의 여백인 패딩(Padding), 박스 모델 사이의 여백인 마진(Margin)으로 구성
...
@media scrren and (min-width: 700px) {
...
- flex-grow, flex-shrink, flex-basis를 한버에 쓸 수 있는 단축 속성

```css
.item {
    flex: 1; /* flex-grow: 1; flex-shrink:1; flex-basis: 0; */
    flex: 1 1 auto; /* flex-grow: 1; flex-shrink: 1; flex-basis: auto; */ 
    flex: 1 500px; /* flex-grow: 1; flex */
}
```
...
- colum의 개수를 미리 정하지 않고 설정된 너비가 허용하는 한 최대한 셀을 채움
...
- 요소가 회전하여 뒷면이 보일 경우 표시 여부를 지정하는 소성
